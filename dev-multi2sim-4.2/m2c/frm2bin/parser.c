...
static const yytype_uint8 yyprhs[] =
{
      ...
      23,    25,    29,    31,    34,    37,    39,    42,    45,    47,
      50,    53,    55,    58,    62,    64,    68,    71,    72,    74,
      78,    80,    82,    84,    86,    88,    90,    92,   100,   104,
     110,   114,   116,   118
};
...
{
      31,     0,    -1,    32,    -1,    22,    32,    -1,    33,    -1,
      33,    32,    -1,    36,    34,    -1,    35,    -1,    35,    34,
      -1,    37,    -1,    41,    -1,    27,    13,    22,    -1,    38,
      -1,    38,    39,    -1,    28,    22,    -1,    40,    -1,    40,
      39,    -1,    12,    22,    -1,    42,    -1,    42,    43,    -1,
      29,    22,    -1,    44,    -1,    44,    43,    -1,    45,    17,
      22,    -1,    22,    -1,     3,    13,    46,    -1,    13,    46,
      -1,    -1,    48,    -1,    48,    14,    46,    -1,     6,    -1,
       7,    -1,     5,    -1,     4,    -1,     9,    -1,    10,    -1,
      47,    -1,    13,    18,    10,    19,    18,    10,    19,    -1,
      18,     6,    19,    -1,    18,     6,    16,    10,    19,    -1,
      18,    10,    19,    -1,    11,    -1,     8,    -1,    13,    -1
};
...
static const char *const yytname[] =
{
  ...
  "text_stmt", "rl_instr", "rl_arg_list", "rl_operand", "rl_arg", 0
};
#endif
...
#line 119 "parser.y"
    {
		struct frm_id_t *id = (yyvsp[(2) - (3)].id);
...
#line 184 "parser.y"
    {
		...
		struct frm2bin_inst_t *inst = (yyvsp[(1) - (3)].inst);
...
#line 297 "parser.y"
    {
  	  	(yyval.arg) = frm_arg_create_scalar_register((yyvsp[(1) - (1)].id)->name);
  	  	frm_id_free((yyvsp[(1) - (1)].id));
  	}
    break;
...
#line 303 "parser.y"
    {
  	  	(yyval.arg) = frm_arg_create_special_register((yyvsp[(1) - (1)].id)->name);
  	  	frm_id_free((yyvsp[(1) - (1)].id));
  	}
    break;
...
#line 309 "parser.y"
    {
		(yyval.arg) = frm_arg_create_zero_register((yyvsp[(1) - (1)].id)->name);
  	  	frm_id_free((yyvsp[(1) - (1)].id));
  	}
    break;
...
#line 315 "parser.y"
    {
  	  	(yyval.arg) = frm_arg_create_predicate_register((yyvsp[(1) - (1)].id)->name);
  	  	frm_id_free((yyvsp[(1) - (1)].id));
  	}
    break;
...
#line 321 "parser.y"
    {
  	  	(yyval.arg) = frm_arg_create_literal((yyvsp[(1) - (1)].num));
  	}
    break;
...
#line 344 "parser.y"
    {
  	  	...
  	  	if (!strcmp((yyvsp[(1) - (7)].id)->name, "c") || !strcmp((yyvsp[(1) - (7)].id)->name, "-c"))
  	  	{
  	  		/* extract the hex value and assign it to the variable */
  	  	  	sscanf((yyvsp[(3) - (7)].id)->name, "%x", &bank_idx);
  	  	  	sscanf((yyvsp[(6) - (7)].id)->name, "%x", &offset);
  	  	  	negative = strcmp((yyvsp[(1) - (7)].id)->name, "c") ? 1 : 0;
  	  	...
  	  	frm_id_free((yyvsp[(1) - (7)].id));
  	  	frm_id_free((yyvsp[(3) - (7)].id));
  	  	frm_id_free((yyvsp[(6) - (7)].id));
  	}
    break;
...
#line 372 "parser.y"
    {
  	  	...
  	  	reg_idx = atoi(((yyvsp[(2) - (3)].id)->name) + 1);
  	  	(yyval.arg) = frm_arg_create_glob_maddr(reg_idx, 0);

  	  	frm_id_free((yyvsp[(2) - (3)].id));
  	}
    break;
...
#line 383 "parser.y"
    {
  	  	...
  	  	int offset;
  	  	/* start from the 2nd character, 1st one is "R" */
  	  	reg_idx = atoi(((yyvsp[(2) - (5)].id)->name) + 1);
  	  	sscanf((yyvsp[(4) - (5)].id)->name, "%x", &offset);
  	  	(yyval.arg) = frm_arg_create_glob_maddr(reg_idx, offset);

  	  	frm_id_free((yyvsp[(2) - (5)].id));
  	  	frm_id_free((yyvsp[(4) - (5)].id));
  	}
    break;
...
#line 397 "parser.y"
    {
  		int offset;
  		sscanf((yyvsp[(2) - (3)].id)->name, "%x", &offset);
...
#line 407 "parser.y"
    {
  	  	(yyval.arg) = frm_arg_create_pt((yyvsp[(1) - (1)].id)->name);
...
#line 415 "parser.y"
    {
  		(yyval.arg) = frm_arg_create_ccop((yyvsp[(1) - (1)].id)->name);
...
#line 423 "parser.y"
    {
  		/* not supported yet */
  	  	frm2bin_yyerror_fmt("unsupported TOK_ID: %s", (yyvsp[(1) - (1)].id)->name);
  	  	frm_id_free((yyvsp[(1) - (1)].id));
